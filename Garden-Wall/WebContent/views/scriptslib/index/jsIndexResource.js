$(document).ready(function() {
	...
	function initmjhyBox(data) {
		...
		for (var i = 0; i < data.length; i++) {
			$("#mjhyBox").append("<li> <div class='accent cobalt '>" + " <a href='javascript:cjxjClickEvent(" + data[i].id + ")'><img class='full' src='views/imagelib/celeBlogImage/" + data[i].srcname + " '/></a> " + " <div class='bottomshowInfor'><span class='icon-white icon-bookmark'></span>" + data[i].bzname + " </div></div> " + " <div class='inforBox'> " + " <span class='tile-title'><span class='icon-white icon-bookmark'></span>" + "<span class='badge  badge-warning'>" + data[i].bzname + "(简介)" + "</span></span> " + data[i].bzintroduce + " </div> </li>");
		}
		//初始化相应的翻转List
		$("#mjhyBox").liveTile({
			startNow : false,
	...
	function gsmjControlEvent() {
		$.commonService("cjhy/queryCjhyResourceByCjxjType", 'POST', {
			cjxjType : 0
		}, function(data) {
			initmjhyBox(data);
		});
